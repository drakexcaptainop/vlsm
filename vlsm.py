...
def experimental(obj_name, alt_method):
    def decor(f):
        def wrapper(*qargs, **kwargs):
            fun = f if args.use_experimental else globals()[obj_name].__dict__[alt_method]
            return fun(*qargs, **kwargs)
        return wrapper
    return decor
...
class addr:
    ...
    def step_next_usable_net(self, for_hosts=None):
        _addr = self.copy()
        _addr.step_optimized( 256 - self.base[-1] )
        if for_hosts is not None:
            _addr._set_mask( host_requested_to_mask(for_hosts) )
        return _addr
    ...
    @experimental( obj_name='addr', alt_method='step' )
    def step_optimized(self, v):
        ...
        self.base[-base_change_index] += max(1, zero_center_div)
...
file = open(args.destination, "w") if args.destination else None

if isinstance(hosts[0], list):
    _addr._set_mask( host_requested_to_mask( sum(hosts[0]) ) )
    for i, hostsarr in enumerate(hosts):
        print(F"VLSM {i+1} - #BaseAddress [{".".join( [str(v) for v in _addr.base] )} / {list2id_mask(_addr.mask)}]:", file=file)
